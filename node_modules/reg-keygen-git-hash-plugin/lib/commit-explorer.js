...
exports.CommitExplorer = void 0;
const git_cmd_client_1 = require("./git-cmd-client");
class CommitExplorer {
    constructor() {
        this._gitCmdClient = new git_cmd_client_1.GitCmdClient();
        this._branchNameCache = {};
    }
    /*
     ...
     */
    getBranchNames(hash) {
        if (this._branchNameCache[hash])
            return this._branchNameCache[hash];
    ...
    getCandidateHashes() {
        const mergedBranches = this.getBranchNames(this._commitNodes[0][0]).filter(b => !b.endsWith("/" + this._branchName) && b !== this._branchName);
    ...
    findBaseCommitHash(candidateHashes, branchHash) {
        const traverseLog = (candidateHash) => {
            if (candidateHash === branchHash)
                return true;
...
exports.CommitExplorer = CommitExplorer;
//# sourceMappingURL=commit-explorer.js.map